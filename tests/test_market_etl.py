...
class TestMarketSilverLayer:
    ...
    def test_daily_return_calculation(self):
        ...
        df = pd.DataFrame({"open": [100.0, 150.0], "close": [110.0, 165.0]})
    ...
    def test_intraday_volatility_calculation(self):
        ...
        df = pd.DataFrame(
            {"open": [100.0, 150.0], "high": [110.0, 165.0], "low": [95.0, 140.0]}
        )
        ...
        assert (
            round(df["intraday_volatility_pct"].iloc[1], 2) == 16.67
        )  # (165-140)/150 * 100
...
class TestMarketGoldLayer:
    ...
    def test_gold_monthly_price_aggregation(self, sample_market_silver_df):
        ...
        agg = (
            sample_market_silver_df.groupby("month_name", observed=True)[
                ["open", "close", "high", "low"]
            ]
            .mean()
            .round(2)
    ...
    def test_gold_volatility_by_day(self, sample_market_silver_df):
        ...
        agg = (
            sample_market_silver_df.groupby("day_name", observed=True)[
...
class TestMarketDataQuality:
    ...
    def test_close_within_range(self, sample_market_silver_df):
        ...
        assert (
            sample_market_silver_df["close"] >= sample_market_silver_df["low"]
        ).all()
        assert (
            sample_market_silver_df["close"] <= sample_market_silver_df["high"]
        ).all()
